...
@test "custom JULIA_BUILD_ROOT: single definition" {
  ...
  touch "${JULIA_BUILD_ROOT}/share/julia-build/1.0.3-test"
  run julia-build --definitions
  assert_success "1.0.3-test"
}

@test "one path via JULIA_BUILD_DEFINITIONS" {
  export JULIA_BUILD_DEFINITIONS="${TMP}/definitions"
  mkdir -p "$JULIA_BUILD_DEFINITIONS"
  touch "${JULIA_BUILD_DEFINITIONS}/1.0.3-test"
  run julia-build --definitions
  assert_success
  assert_output_contains "1.0.3-test"
...
@test "multiple paths via JULIA_BUILD_DEFINITIONS" {
  ...
  touch "${TMP}/definitions/1.0.3-test"
  mkdir -p "${TMP}/other"
  touch "${TMP}/other/1.0.4-test"
  run julia-build --definitions
  assert_success
  assert_output_contains "1.0.3-test"
  assert_output_contains "1.0.4-test"
...
@test "installing definition from JULIA_BUILD_DEFINITIONS by priority" {
  export JULIA_BUILD_DEFINITIONS="${TMP}/definitions:${TMP}/other"
  mkdir -p "${TMP}/definitions"
  echo true > "${TMP}/definitions/1.0.3-test"
  mkdir -p "${TMP}/other"
  echo false > "${TMP}/other/1.0.3-test"
  run bin/julia-build "1.0.3-test" "${TMP}/install"
  assert_success ""
}
...
@test "removing duplicate Julia versions" {
  ...
  mkdir -p "$JULIA_BUILD_DEFINITIONS"
  touch "${JULIA_BUILD_DEFINITIONS}/1.0.3"
